...
def predict_images(img_path, network, model_path):
	...
	for img_info in all_imgs: # sorted(os.listdir(img_path))):
		...
		for key in bboxes:
			...
			for jk in range(new_boxes.shape[0]):
				...
				candidate_rois.loc[len(candidate_rois)] = [ img_name, key, new_probs[jk], width, height, real_x1, real_y1, real_x2, real_y2]
...
predict_images(options.test_path, options.network, options.load)