...
class demographics():
    ...
    def piechart_preterm(values):
        demographics.piechart_binary(values=values, labels=[
                                     'Preterms', 'Non-preterms'])
    ...
    def hist_gw(ga_week):
        ...
        hist, bins = np.histogram(
            ga_week, bins=40, range=(22, 42), density=True)
        # hist = np.append(hist, 0)
        # bins = 0.5 * (bins[:-1] + bins[1:])
        fig = px.bar(x=bins[:-1], y=hist/hist.sum() * 100,
                     labels={'x': 'Gestational Age (Weeks)', 'y': 'Percentage (%)'})
    ...
    def hist_bw(bw_kg):
        ...
        # bins = 0.5 * (bins[:-1] + bins[1:])
        fig = px.bar(x=bins[:-1], y=hist/hist.sum() * 100,
                     labels={'x': 'Birth Weight(kg)', 'y': 'Percentage (%)'})
        # fig = px.histogram(data, x="total_bill", histnorm='probability density')
        fig.update_layout(plot_bgcolor="white")
        fig.update_traces(marker_color='mediumturquoise', marker_line_color='rgb(8,48,107)',
...
class annotation():
    '''
    Provides static ploting and detection functions for fundus image annotations.    
    '''
    ...
    def show_anno(filepath, df, savefolder=None, drawzones=False, showimg=True, suffix='_DETECTION'):
        ...
        for _, row in df.iloc[idxs].iterrows():
            ...
            zone2p = [cx-1.3*radius, cx+1.3*radius,
                      cy-1.3*radius, cy+1.3*radius]
            ...
            if (ymin < 5):
                xy = (max(1, xmin), ymax)
            ax.annotate(row['class'] + prob, xy=xy,
                        color=edgecolor, fontsize=round(im_width * 0.024))

            # add bounding boxes to the image
            rect = patches.Rectangle(
                (xmin, ymin), width, height, edgecolor=edgecolor, facecolor='none')
    ...
    def rule_filter_rois(input_file='../src/odn/candidate_rois.txt',
        ...
        for index, fn in enumerate(uniquefiles):
            ...
            if (len(ods) > 0):
                ...
                to_be_removed = np.append(
                    to_be_removed, ods[ods.index != ods['prob'].idxmax()].seq.values.flatten())
                ...
                if len(mas) > 0:
                    ...
                    # print(ma_dict)
                    to_be_removed = np.append(
                        to_be_removed, mas[mas.index != idx_least_error].seq.values.flatten())
                ...
                if (len(mas) > 0):
                    ...
                    to_be_removed = np.append(
                        to_be_removed, mas[mas.index != mas['prob'].idxmax()].seq.values.flatten())

            rows.drop(rows.loc[rows.seq.isin(
                to_be_removed)].index, inplace=True)
            all_to_be_removed = np.concatenate(
                (all_to_be_removed, to_be_removed))
        ...
        rois.drop(rois.loc[rois.seq.isin(
            all_to_be_removed)].index, inplace=True)
    ...
    def calculate_fundus_zones(classes, scores, boxes, threshold=0):
        ...
        zone2p = [cx-1.3*radius, cx+1.3*radius,
                  cy-1.3*radius*4/3, cy+1.3*radius*4/3]
    ...
    def draw_fundus_zones_on_image_array(image, zones, text='', use_normalized_coordinates=True):
        ...
        for zone in zones:
            ...
            draw.ellipse((int(left), int(top), int(right), int(bottom)),
                         fill=None, outline=colors[idx])   # (255,255,255)
            ...
            with tf.Session(graph=detection_graph) as sess:
                ...
                with tqdm(total=len(FILES)) as pbar:
                    for image_path in FILES:
                        ...
                        # for png, convert rbga to rbg (3ch)
                        if (image_path.lower().endswith('.png')):
                            image = image.convert('RGB')

                        if new_img_width is not None and new_img_width > 0:
                            # Image.ANTIALIAS
                            image.thumbnail((new_img_width, new_img_width))
                        ...
                        zones = annotation.calculate_fundus_zones(
                            classes[0], scores[0], boxes[0], threshold)
                        annotation.draw_fundus_zones_on_image_array(
                            image_np, zones=zones, text=label)
                        ...
                        # the validity of log_file is already checked in the beginning
                        if (log_file is not None and log_file != ''):
        ...
        for path, im, im0s, _, _ in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                annotator = plots.Annotator(
                    im0, pil=anno_pil, line_width=None, example=str(names))  # pil=True / False
                ...
                cx, cy, radius = annotation.torch_calculate_fundus_zones(
                    fundus_classes, fundus_scores, fundus_bbox)
                ...
                if cx > 0 and cy > 0:  # cx and cy = -1 if no optic disc is found
                    ...
                    # means both optic disc and macula are found. use 2*|| od - macula || as the zone I radius.
                    elif radius > 0:
                        radii.append(radius)
                ...
                if output_path == 'radius':
                    ...
                    target_path = path.replace(pathlib.Path(
                        path).suffix, suffix + pathlib.Path(path).suffix)
                    cv2.imwrite(target_path, im0)
                else:
                    os.makedirs(output_path, exist_ok=True)
                    target_path = output_path + '/' + os.path.basename(path).replace(
                        pathlib.Path(path).suffix, suffix + pathlib.Path(path).suffix)
                    cv2.imwrite(target_path, im0)
    ...
    def torch_batch_object_detection_for_one_exam(model_path='../src/odn/torch_yolo/runs/train/exp15/weights/best.pt',
        ...
        Return
        ------
        radius - the radius of the fundus Zone I
        '''
...
class dataset():
    '''
    Dataset manipulation class for fundus images.
    '''
    ...
    def synthesize_anno(label_file,
        ...
        for f in unique_files:
            ...
            filepath = dataset.get_full_path_by_filename(FILES, f)
    ...
    def split(label_file, dir_images, train_output, test_output,
        ...
        test_set = df.loc[df['filename'].isin(test_files)].copy().sort_values(by=[
            'filename']).reset_index(drop=True)
        train_set = df.loc[df['filename'].isin(test_files) == False].copy(
        ...
        for i in range(data.shape[0]):
            ...
            data['format'][i] = '../' + dir_images + data['format'][i] + ',' + str(train_set['xmin'][i]) + ',' + str(
                train_set['ymin'][i]) + ',' + str(train_set['xmax'][i]) + ',' + str(train_set['ymax'][i]) + ',' + train_set['class'][i]
            # print(data['format'][i])
        ...
        for i in range(data.shape[0]):
            data['format'][i] = '../' + dir_images + data['format'][i] + ',' + str(test_set['xmin'][i]) + ',' + str(
                test_set['ymin'][i]) + ',' + str(test_set['xmax'][i]) + ',' + str(test_set['ymax'][i]) + ',' + test_set['class'][i]
    ...
    def add_new_row(img_, bboxes_, row, idx,
        ...
        # .replace('.jpg', '.' + str(idx) + '.jpg', 1)
        new_path = row[0].replace(old_image_path_seg, new_image_path_seg, 1)
        _ = cv2.imwrite(new_path, img_[:, :, ::-1])  # bgr -> rgb
        ...
        for index, row in df.iterrows():
            ...
            if AUG_FLIP:
                ...
                raise NotImplementedError(
                    __class__.__name__ + ' TODO: 把多个bbox和标签放到一起做aug操作')

                # FLIP
                img_, bboxes_ = RandomHorizontalFlip(
                    1)(img.copy(), bboxes.copy())
            ...
            if AUG_COMBO:  # FLIP + SCALE + TRANSLATE + ROTATE
                ...
                for idx in range(2, 7):
                    try:
                        img_, bboxes_ = RandomHorizontalFlip(
                            0.5)(img.copy(), bboxes.copy())
                        img_, bboxes_ = RandomScale(
                            0.1, diff=False)(img_, bboxes_)
        ...
        for root, dirs, files in os.walk(folder):
            for f in files:
                ...
                if prefix != '' and prefix is not None and f.startswith(prefix):
                    continue
                dataset.resize_fundus_image(root, f, target=target,
                                            prefix=prefix, w=w, h=h)
        ...
        for idx, item in enumerate(json_annos):
            if 'laterality' in item.keys():
                ...
                if lcode and not lcode.isspace():
                    file = item['filename']
                    shutil.copyfile(os.path.join(IMAGEDIR, file),
                                    os.path.join(L2DIR, lcode, file))
    ...
    def expand_images_by_flipping(imagedir):
        ...
        for root, dirs, files in os.walk(imagedir):
            for f in files:
                if f.endswith('.jpg'):
                    im = Image.open(os.path.join(root, f)).transpose(
    ...
    def resize_fundus_image(root, f, target, prefix='', w=480, h=360):
        ...
        new_file_path = os.path.join(target, prefix + f)
        im = Image.open(file_path).resize((w, h))
