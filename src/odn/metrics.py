...
def bbox_RCE(boxA, boxB, width, height):
    ...
    delta_area = math.sqrt((cxA - cxB)**2 + (cyA - cyB)**2)

    return delta_area / diag
...
def fundus_metrics(gt='../data/fundus/all_labels.csv',
    ...
    for fn in set(rois['filename'].values):
        ...
        bbox_opticdisk_gts = df.loc[(df['filename'] == fn) & (df['class'] == 'OpticDisk')][[
            'xmin', 'ymin', 'xmax', 'ymax', 'width', 'height']].values
        ...
        bbox_opticdisk_preds = rois.loc[(rois['filename'] == fn) & (
            rois['class'] == 'OpticDisk')][['xmin', 'ymin', 'xmax', 'ymax', 'prob']].values
        ...
        if (bbox_opticdisk_gt.tolist() == [0, 0, 0, 0] and bbox_opticdisk_pred.tolist() == [0, 0, 0, 0]):
            # there is acturally no this structure and we didn't detect it.
            IOU_O[fn] = 1
        else:
            IOU_O[fn] = bbox_IoU(bbox_opticdisk_gt, bbox_opticdisk_pred)
        RCE_O[fn] = bbox_RCE(
            bbox_opticdisk_gt, bbox_opticdisk_pred, width, height)
        P_O[fn] = prob

        # IOU for Macula

        bbox_macula_gt = [0, 0, 0, 0]
        bbox_macula_gts = df.loc[(df['filename'] == fn) & (df['class'] == 'Macula')][[
        ...
        bbox_macula_preds = rois.loc[(rois['filename'] == fn) & (
            rois['class'] == 'Macula')][['xmin', 'ymin', 'xmax', 'ymax', 'prob']].values
...
def fundus_compare_metrics(gt='../data/fundus/all_labels.csv',
    ...
    fig, ax = plt.subplots(n, len(image_dirs) + 1,
                           figsize=(16 * len(image_dirs), 9 * n))
    ...
    for row, fn in enumerate(image_subset):
        ...
        if verbose:
            ...
            text = fn[:20] + '\n' + fn[20:] + '\n\n'  # manually break word
            text += 'Probability of Optic disc: ' + \
                    str(round(P_O[fn], 3)) + '\n'
        ...
        for col in range(len(image_dirs) + 1):
            ...
            if (col < len(image_dirs)):
                ...
                # print(text)
                ax[row, col].text(
                    0.1, 0.2, text, transform=ax[row, col].transAxes, size=45, wrap=True)

    # fig.tight_layout()
    plt.subplots_adjust(left=0, bottom=0, right=1,
                        top=1, wspace=-0.1, hspace=-0.1)
...
def fundus_compare_metrics_separate(input_file='../src/odn/rois.txt',
    ...
    for row, fn in enumerate(uniquefiles):
        ...
        for col, image_dir in enumerate(image_dirs):

            ax[col].set_axis_off()
            image = plt.imread(image_dir + '/' + fn)
            crop_ratio = 0.05
            img_cropped = image[(int)(image.shape[0] * crop_ratio):(int)(image.shape[0] * (1-crop_ratio)),
...
def metric_histogram(gt='../data/fundus/all_labels.csv',
    ...
    labels = ['IoU of Optic Disc', 'RCE of Optic Disc', 'ACC of Optic Disc',
              'IoU of Macula', 'RCE of Macula', 'ACC of Macula']
    IOU_O, IOU_M,	RCE_O, RCE_M, P_O, P_M = fundus_metrics(gt, pred)

    # use P_O and P_M to replace ACC_O and ACC_M
    for idx, M in enumerate([IOU_O, RCE_O, P_O, IOU_M, RCE_M, P_M]):
        row = idx // 3
        col = idx % 3
        ...
        _ = ax[row, col].hist(m, label=labels[idx], bins=10,
...
def fundus_compare_metrics_html2(gt='../data/fundus/all_labels.csv',
    ...
    for row, fn in enumerate(uniquefiles):
        ...
        for idx, m in enumerate(SHOWSET):
            if str(round(m[fn], 3)) != str(round(SHOWSET_2[idx][fn], 3)):
                s += '<tr>'
                s += '<td>' + str(row) + '</td>'
                s += '<td>' + fn + '</td>'
                for _, m1 in enumerate(SHOWSET):
                    s += '<td>' + str(round(m1[fn], 3)) + '</td>'
                    # s+= '<td>' + str(round(SHOWSET_2[idx][fn],3)) + '</td>'
                    # print(round(SHOWSET_2[0][fn],3))
                for _, m2 in enumerate(SHOWSET_2):
                    # s+= '<td>' + str(round(SHOWSET[idx][fn],3)) + '</td>'
                    s += '<td>' + str(round(m2[fn], 3)) + '</td>'
                s += '</tr>'
